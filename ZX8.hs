...
id56s :: [Identity]
id56s =  map toId id56s' where
  id56s' = map (\xs -> map (\(F.G i ws) -> (sfl ws, i)) xs) (F.id56s [0..5])


id67s :: [Identity]
id67s =  map toId id67s' where
  id67s' = map (\xs -> map (\(F.G i ws) -> (sfl ws, i)) xs) (F.id67s [0..6])

id6s :: [Identity]
id6s =  map toId id6s' where
  id6s' = map (\xs -> map (\(F.G i ws) -> (sfl ws, i)) xs) (drop 1 $ F.id6s [0..5])

id45s :: [Identity]
id45s =  map toId id45s' where
  id45s' = map (\xs -> map (\(F.G i ws) -> (sfl ws, i)) xs) F.i45eff
...
wid96' = ids where
  ...
  ids3 = [toId $ idn 5 ++ idn 4 ++ idnw [1..4]]
    ++ map idn3 [6..25] 
...
wid49 = ids where
  ids1 = map toId $ [idn 4, idn 5, idn 6, idn 7]
  ids3 = [idn3 6, idn3 7, idn3 8, idn3 9, idn3 10]
...
idn n = ids' where
  inds = [0..n-1]
  ids' =  [  (sfl [x], ((((n-2)*(n-3)) `div` 2)`mod` 8)) | x <- inds ]
...
instance (Inv a) => Inv [a] where
  inv cir = [concat $ map inv (reverse cir)]

inv_cir :: [Gate] -> [Gate]
inv_cir cir = concat $ map inv_g (reverse cir)
...
-- same wires.
toGads :: [Gadget] -> Gads
toGads gs = MS.fromListWith (\x y -> (x+y) `mod` 8) gs
...
mypartition p xs = (l,r) where
  l = MS.filter (\x -> x `mod` 2 == 1) xs
  r = MS.filter (\x -> x `mod` 2 == 0) xs  

tgcg2distr :: TGCG -> Distribution Int
tgcg2distr (tg,cg) = d where
  ws = wiresOfGads tg
  sws = length ws
...
sampleNwire seed n distr = ws where
  gen = fromDistribution distr
  sampleN 0 d g = ([],g)
  sampleN n d g = let (w,g') = sample (fromDistribution d) g in
    let (ws, g'') = sampleN (n-1) (assuming (\x -> x /= w) d) g' in
      (w:ws, g'')
...
sampleNwire' :: Int -> Int -> Distribution Int -> [Int]
sampleNwire' seed n distr = ws where
  ws = if  length (nub ws') /= n then sampleNwire' (seed+1) n distr
    else ws'
  gen = fromDistribution distr
  sampleN 0 d g = ([],g)
  sampleN n d g = let (w,g') = sample (fromDistribution d) g in
    let (ws, g'') = sampleN (n-1) d g' in (w:ws, g'')
  ws' = fst $ sampleN n distr (mkStdGen seed)


sampleNw :: StdGen -> Int -> Distribution Int -> ([Int], StdGen)
sampleNw stdg n di = (ws, stdg') where
  (ws'', stdg'') = sample_nrep' n stdg di
  (ws, stdg' ) = if length (nub ws'') < n then
    sampleNw stdg'' n di
    else
    (ws'', stdg'')
...
sampleNws stdg m n di = (xss, stdg') where
...
--    (tol', stdg') =  randomR (0,tol) stdg
    tol' = tol
    nt' = foldl'
      (\a x -> if MS.lookup (fst x) tm /= Nothing
        then a
        else a + 1
      ) 0 (take (dlen+tol+1) (MS.toList tg))
    tgintm = map (\x -> (x, MS.lookup (fst x) tm))(MS.toList tg)    
    (jkeys, nkeys) = partition (\(x,y) -> y /= Nothing) tgintm
    nt = length $ jkeys 
    jkeys' = map fst jkeys
    nkeys' = map fst nkeys    
    tm1 = foldl' (\ys x -> MS.delete (fst x) ys) tm jkeys'
    tm' = foldl' (\ys x -> MS.insert (fst x) (snd x) ys) tm1 nkeys'
    cm1 = foldl' (\ys x -> MS.insertWith pmod8 (fst x) (snd x) ys) cm
      $ (map (\(x,y) -> (fst x, pmod8 (snd x) (unJust y))) jkeys) 
    cm' = foldl' (\ys x -> MS.insertWith pmod8 (fst x) (snd x) ys) cm1 $ MS.toList cg


tryIds :: StdGen -> Int -> [Identity] -> TGCG -> (TGCG, StdGen)
tryIds stdgen tol ids mm =
  foldl' (\mmg x -> {-# SCC "TryId-" #-} tryId (snd mmg) tol x (fst mmg) )
    (mm,stdgen) ids 
...
bmask' n g
  | n == 0 = ([],g)
  | otherwise =
    let (a,g') = sample (fromDistribution (uniform [True,False])) g in
      let (as, g'') = bmask' (n - 1) g' in
        (a:as, g'')
...
id4ton seed n
  ...
  where
    ws = [0..n-1]
    fs = [4..n]
    wss = concat $ map (\x -> choosen_linear x ws) fs
    bm = bmask seed (length wss -1)
    bmwss = zip wss (bm ++ [True])
    wss' = map fst $ filter (\(x,y)->y) bmwss
    idns = concat $ map idnw wss'
...
id4ton_dis seed n len
  ...
  where
    ...
    fs = [4..n]
    wss = concat $ map (\x -> choosen_linear x ws) fs
    wss' = fst $ sample_nrep (len) (mkStdGen seed)
      (fromList freq)
    tot = sum (map (\x -> exp $ fromIntegral $ length x) wss)
...
sample_rep n g gen
  | n == 0 = ([], g)
  | otherwise =
    let (a,g') = sample gen g in
      let (as, g'') = sample_rep (n - 1) g' gen in
        (a:as, g'')
...
sample_nrep n g dis
  | n == 0 = ([], g)
  | otherwise =
    let (a,g') = sample (fromDistribution dis) g in
      let (as, g'') = sample_nrep (n - 1) g' (assuming (\x -> x /= a) dis) in (a:as, g'')
...
expandId id@(tg,cg) wss =
  [(MS.fromList ( map (\(k,v) -> (f k, v)) (MS.toList tg)),
    (MS.fromList   (map (\(k,v) -> (f k, v))(MS.toList cg)))) | f <- candisfs]
  where
...
choosen_linear n xs
  | length xs < n = []
  | length xs == n = [xs]  
choosen_linear n xs@(h:t) = ys : zss where
  ys = take n xs
...
pair2fun [] x = x
pair2fun p x = case lookup x p of
  Nothing -> x
  Just y -> y
...
runIds_sa stdgen rep tol (h:t) mm = (mm'', stdgen') where   
...
runIds_saw stdgen rep tol woglen (h:t) mm = (mm'', stdgen') where   
  len = length (wiresOfId h)
...
runIds_saf stdgen tol ids@(h:t) mm = (mm'', stdgen') where   
  len = length (wiresOfId h)
  wog = wiresOfGads $ fst mm
...
runIds_safr stdgen tol ids mm = (mm', stdg') where   
  (mm1, stdg1) = runIds_saf stdgen tol ids mm
  (mm', stdg') = case mm1 == mm of
    True -> (mm, stdg1)
    False -> runIds_safr stdg1 tol ids mm1


zxid45s = map toId ids' where
  w4s = F.choosen 4 [0..4] ++ F.choosen 5 [0..4]
  w4se = map (\x -> [[],x]) w4s
  w45' = [idnw x1 ++ idnw x2 ++ idnw x3 ++ idnw x4 ++ idnw x5 ++ idnw x6 |
          x1 <-w4se!!0, x2 <-w4se!!1, x3 <-w4se!!2, x4 <-w4se!!3, x5 <-w4se!!4, x6 <-w4se!!5 ]
  ids' =  drop 1 w45'

runIds_r :: StdGen -> Int  ->  [Identity] -> TGCG -> LMMR
runIds_r stdgen rep ids mm = mm' where
  tols = [0] :: [Int]
  rts = map (\x ->  \(a, g) -> {-# SCC "runIds_sa-" #-} runIds_saf g x ids a) tols
  ct = foldl (.) id rts
  ct' = case length wog <= 21 of
    False -> \(a, g) -> {-# SCC "runIds_sa-" #-} runIds_sa g rep 0 ids a
    True -> \(a, g) -> {-# SCC "runIds_sa-" #-} runIds_safr g 0 ids a
...
runIds_rw stdgen rep ids mm = do
  ...
  let ct = foldl (.) id rts
  let ct' = case  fq <= 27 of
        False -> \(a, g) -> {-# SCC "runIds_sa-" #-} runIds_saw g rep 0 fq ids a
        True -> \(a, g) -> {-# SCC "runIds_sa-" #-} runIds_safr g 0 ids a
...
instance F.PDFable (LMR) where
  topdf lmr = F.topdf (F.LMR l m r) where
    (a,(s', vqfq, tct)) = runState lmr (([],[]),(0,0),(0,0,0))
    s =  s'  
    l =  fst s
    m =  a
    r =  snd s
  topdf_file lmr = F.topdf_file (F.LMR l m r) where
    (a,(s', vqfq, tct)) = runState lmr (([],[]),(0,0),(0,0,0))
    s =  s'  
    l =  fst s
    m =  a
...
instance F.PDFable (LMMR) where
  topdf lmmr = F.topdf (F.LMR l m r) where
...
gads2cir' xs = sortBy mycmp $ concat $ map gad2cir' xs
...
gads2cir :: [Gadget] -> [Gate]
gads2cir xs = concat $ map gad2cir xs
...
zx2cir1 x = [x]
zx2cir cir = concat $ map zx2cir1 cir
...
cir2zx cir = concat (map g2zx cir)
...
movecx_Left xs = (reverse lcx, reverse lg) where
  (lcx,lg) = execState (mvcx_left xs) ([],[])


mymvcx :: [Gate] -> ([Gate], [Gate])
mymvcx [] = ([],[])
mymvcx (CX i j : t) = (tg', CX i j : tcx) where
  (tg, tcx) = mymvcx t
  tg' = movecx_left' (CX i j) tg
mymvcx (h : t) = (h : tg, tcx) where
  (tg, tcx) = mymvcx t

-- | tail recursive
mvcxtc :: [Gate] -> ([Gate], [Gate]) -> ([Gate], [Gate]) 
mvcxtc [] (mg,rcx) = (mg,rcx)
mvcxtc (CX i j : t) (mg,rcx)  = mvcxtc t (mg', CX i j : rcx)  where
...
mvcx_left xs = foldM' movecx_left2 () xs
...
movecx_left' (CX i j) (Ga p ws : tt) = (Ga p ws') : (movecx_left' (CX i j) tt)
  where
    ws' = case i `member` ws of
      True -> case j `member` ws of
        True -> Set.delete j ws
        False -> Set.insert j ws
      False -> ws
...
-- | cxccx 
cir2lmmr_cxccx cir = initLMR cir >>=  (moveCXCCX_half_squee ) >>= (\x -> moveH_old $ F.desugar_cir x) >>=  decompH  >>= cir2lxr >>= pickClifford >>= moveX  >>= moveSwap  >>=  (\x ->  {-# SCC "moveCX'-" #-} moveCX' x)    >>=  tolmmr  



--cir2lmmr :: [Gate] -> LMMR
cir2lmmr cir = (initLMR (F.desugar_cir cir)) >>= (moveH) >>= decompH >>= cir2lxr >>= moveSwap  >>= moveX   >>= ( \x ->  moveCX' x)


cir2lmmr' cir = (moveH (F.desugar_cir cir))  >>=  decompH  >>= cir2lxr >>= pickClifford >>= moveX  >>= moveSwap  >>=  (\x ->  {-# SCC "moveCX'-" #-} moveCX' x)    >>=  tolmmr  

cir2lmmr'2 cir = (moveH cir)  >>=  decompH  >>= cir2lxr >>= pickClifford >>= moveX   >>= moveSwap  >>= (\xs -> tolmmr $ filter (not.isCX) xs)

gf_split :: [Gate] -> ([Gate],[Gate],[Gate])
gf_split cir = (l,m,r) where
...
tcount_zx :: LMR -> Int
tcount_zx xs = length $ filter isTGad xs' where
